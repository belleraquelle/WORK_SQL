...
The query below will return CRNs that meet the criteria and which have no missing marks / grades.
In theory, these should be ready to roll to academic history.
...
WHERE
    ...
    -- Only include CRNs with no missing marks
    AND shrmrks_crn NOT IN (
    	SELECT 
    		shrmrks_crn
    	FROM 
    		shrmrks
