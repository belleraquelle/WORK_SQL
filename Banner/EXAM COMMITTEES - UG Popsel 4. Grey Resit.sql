...
WHERE
    ...
    --Limit to students with an expected completion date beyond the UCF date
    AND s1.sorlcur_end_date > :EX_0_UCF_DATE
    
    --Exclude students on a current X1 attribute or currently on a Foundation Year
	AND (s1.sorlcur_pidm||s1.sorlcur_key_seqno NOT IN (
		SELECT
    ...
    AND s1.sorlcur_program NOT IN ('FNDIP-FBE','FNDIP-FBU','FNDIP-FCO','FNDIP-FEG','FNDIP-FHU','FNDIP-FLL', 'FNDIP-LSF')
    )
    
    -- Exclude students with pending awards
    AND s1.sorlcur_pidm NOT IN (
        SELECT
            spriden_pidm
        FROM 
    		shrdgmr,
