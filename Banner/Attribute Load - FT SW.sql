/*
 * Attribute Loading - X2A for FT SW students
 * 
 */
...
SELECT DISTINCT
    ...
    SUBSTR(smralib_area,-4,4) AS "SW_Type",
...
WHERE
    ...
    -- Limit to students on programmes with L4 SW areas
    AND SUBSTR(smralib_area, -4, 4) = 'L5SW'
    
    -- Limit to programmes with X based progression stages
    AND smrpaap_program IN (
    	SELECT j1.smrpaap_program
    	FROM smrpaap j1 JOIN smralib k1 ON j1.smrpaap_area = k1.smralib_area
    	WHERE k1.smralib_atts_code IN ('X2','X2A')
    )
    
    -- Limit to full-time students
    AND t2.sorlcur_styp_code = 'F'
    
    -- Limit to Sandwich students
    
    AND  z1.sgrstsp_pidm || z1.sgrstsp_key_seqno IN (
